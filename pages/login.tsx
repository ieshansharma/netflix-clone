  ...
  return (
    <div className="relative flex h-screen w-screen flex-col bg-black md:items-center md:justify-center md:bg-transparent">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="relative mt-24 space-y-8 rounded bg-black/75 py-10 px-6 md:mt-0 md:max-w-md md:px-14">
        ...
        <label className="inline-block w-full">
        <input type='password' placeholder="Password" {...register('password', {required:true})} className="input"/>
        {errors.password && (
              <p className="p-1 text-[13px] font-light  text-orange-500">
        ...
        <div className="text-[gray]">
          New To Netflix?{' '}
          <button type='submit' onClick={() => setLogin(false)} className="text-white hover:underline">Sign Up now</button>
        </div>
      </form>
    </div>
  )
